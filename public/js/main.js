...
if(gardenSub) {
	gardenSub.addEventListener("click", function(e) {
	    ...
	    axios.post('home/newgarden', postGarden).then((response)=>{
	    	// console.log(response);
	        if(response.data == 'All good') {
	       		window.location.href = "/home";
	        }else if(response.data == "Garden height and width must be a number greater than 0 ft") {
	        	errorId.innerHTML = response.data;
	        	errorId.style.opacity = "1";
	        }else if(response.data == "Please fill in all fields") {
	        	errorId.innerHTML = response.data;
	        	errorId.style.opacity = "1";
	        }else if(response.data == "Garden name must be less than 28 characters") {
	        	errorId.innerHTML = response.data;
	        	errorId.style.opacity = "1";
	        }
	    }).catch((error)=>{
...
function makeGarden(h, w,  jsonres) {
	if(window.innerWidth < 768) {
		for(var i = 0; i<(w*h); i++) {
			gardenPlot.innerHTML += '<div id="square'+i+'" class="plantDiv"><div class="plantFix"></div></div>';
	...
	for(var i = 0; i < gardenElementsArr.length; i++) {
		gardenElementsArr[i].innerHTML += '<img class="vegPic" src='+globalUrl+'/'+names[i]+'.svg }}" /><p class="countImg"></p>';
...
function makeList(jsonres2) {
	...
	for(var i = 0; i < unique_array.length; i++) {
		list.innerHTML += "<tr><td><img src='"+globalUrl+"/"+unique_array[i]+".svg'></td><td class='middleRow'>"+(count[unique_array[i]]*newQuant[i])+" "+newClassify[i]+"</td><td class='lastRow'>"+((count[unique_array[i]]*newQuant[i])*newYield[i])+"</td></tr>";
	}
...
function addPlant(e) {
	...
	target.path[1].innerHTML += "<img class='vegPic' src="+globalUrl+"/"+e.target.parentNode.childNodes[5].innerHTML+".svg />";
	options.classList.remove('optionsToggle');
...
if(removePlant) {
	removePlant.addEventListener('click', function(e) {
...
function saveGarden(e) {
    ...
    // send contact form data.
    axios.post('test/save/'+difStr, test2).then((response)=>{
    // console.log(response);
...
if(deleteGard) {
	...
	for(var i = 0; i < deleteGard.length; i++) {
		deleteGard[i].addEventListener("click", function(e) {
			e.stopPropagation();
			zindexFix = e.target;
	...
	for(var i = 0; i < archiveButton.length; i++) {
		archiveButton[i].addEventListener("click", function(e) {
			e.stopPropagation();
...
if(restoreButtonFix) {
	for(var i = 0; i < restoreButtonFix.length; i++) {
		restoreButtonFix[i].addEventListener('click', function(e) {
